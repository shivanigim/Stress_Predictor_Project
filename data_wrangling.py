...
#pd1 = pd.read_pickle(r'D:\WESADProj\S2\S2.pkl')
pd1 = pd.read_pickle('D:\WESAD_RawData\S2\S2.pkl')

#pd1

# E4 (wrist) Sampling Frequencies
fs_dict = {'ACC': 32, 'BVP': 64, 'EDA': 4, 'TEMP': 4, 'label': 700, 'Resp': 700}
# where 32,64... are the frequecies in hertz for each respective sensor(no of observation made per second)
...
PICKLE_FILE_PATH='../WESAD_RawData'
...
def compute_features(e4_data_dict, labels, norm_type=None):
    ...
    # Change indices to datetime in seconds
...
def get_samples(data, n_windows, label):
    ...
    for i in range(n_windows):
        ...
        w.to_csv("test_case_files/"+str(TESTCASE_SEQ)+".csv")
        #testno.append(TESTCASE_SEQ)
        ...
        wdf['TestCaseId'] = TESTCASE_SEQ
        samples.append(wdf)

#    newD = pd.DataFrame(testno)
#    newD.to_csv('testno.csv')
...
def make_patient_data(subject_id):
    ...
    subject = SubjectData(main_path=PICKLE_FILE_PATH, subject_number=subject_id)
